...
export default {
  ...
  created() {
    this.getSignalDuration();
  },
  methods: {
    async getSignalDuration() {
      this.loading = true;
      await signalExtraData()
        .then((response) => {
          this.rows = response.data.signal_time;
          this.loading = false;
        })
        .catch((error) => {
          this.$store.dispatch(
    ...
    changeSingleDuration(selected) {
      let assinment = `signal_time,${selected.title}`;
      this.$emit("selectedDuration", assinment);
    },
  },
};
</script>
